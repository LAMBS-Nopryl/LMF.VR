...
if (typeOf _supp == var_supSmall) exitWith {
	_supp addItemCargoGlobal ["ACE_Wirecutter",1];
	if (_LAT != "") then {_supp addWeaponCargoGlobal [_LAT, 2];};
	if (_Rifle_Ammo != "") then {_supp addMagazineCargoGlobal [_Rifle_Ammo, 20];};
	if (_Carbine_Ammo != "") then {_supp addMagazineCargoGlobal [_Carbine_Ammo, 10];};
	if (_Rifle_GL_Ammo != "") then {_supp addMagazineCargoGlobal [_Rifle_GL_Ammo, 10];};
	if (_LMG_Ammo != "") then {_supp addMagazineCargoGlobal [_LMG_Ammo, 4];};
	if (_Rifle_GL_HE != "") then {_supp addMagazineCargoGlobal [_Rifle_GL_HE, 10];};
	if (_Rifle_GL_Flare != "") then {_supp addMagazineCargoGlobal [_Rifle_GL_Flare, 10];};
};
...
if (typeOf _supp == var_supLarge) exitWith {
	...
	if (_Grenade_Smoke_Alt != "") then {_supp addMagazineCargoGlobal [_Grenade_Smoke_Alt, 10];};
	if (_LAT != "") then {_supp addWeaponCargoGlobal [_LAT, 6];};
	_supp addItemCargoGlobal ["ACE_Wirecutter",5];
};

if (typeOf _supp == var_supMedical) exitWith {
	_supp addItemCargoGlobal ["ACE_morphine",30];
	_supp addItemCargoGlobal ["ACE_epinephrine",30];
	_supp addItemCargoGlobal ["ACE_tourniquet",30];
	_supp addItemCargoGlobal ["ACE_splint",30];
	_supp addItemCargoGlobal ["ACE_bloodIV",15];
	_supp addItemCargoGlobal ["ACE_packingBandage",50];
};

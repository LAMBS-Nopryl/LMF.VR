...
private _a2k_staticReset = {
	...
	if ((vehicle _gunner) == _gunner) then {(group _gunner) setCombatMode "GREEN";};

	_forgetTheseTargets = _gunner targets [true, 1000];
	{(group _gunner) forgetTarget _x} forEach _forgetTheseTargets;	
};
...
while {{alive _x} count (units _grp) > 1} do {
	...
	if (_type == "MORTAR") then {
		...
		//BARRAGE MAIN LOOP
		while {{alive _x} count (units _grp) > 1 && {!(isNull (_gunner findNearestEnemy _gunner))} && {count (magazines (vehicle _gunner)) != 0}} do {
			_grp setCombatMode "BLUE";
			...
			if (vehicle _gunner != _gunner && {canFire (vehicle _gunner)} && {_targetPos inRangeOfArtillery [[(vehicle _gunner)], currentMagazine (vehicle _gunner)]}) then {
				...
				for "_i" from 0 to (3 + (random 5)) do {
					...
					private _forgetTheseTargets = (leader _grp) targets [true, 1000];
	...
	waitUntil {sleep 5; isNull (_gunner findNearestEnemy _gunner) || {count (magazines (vehicle _gunner)) == 0} || {{alive _x} count units _grp < 1}};
